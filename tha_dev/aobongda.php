...
<table id="table">
  <?php
				...
				mysql_select_db("freev_12083797_sanpham",$conn);
				$display=20;
						if (isset($_GET['id']) && (int)$_GET['id']) $p=intval($_GET['id']);
						else $p=1;
							$countqr=mysql_query('SELECT count(loai) FROM san_pham where loai="Áo Câu Lạc Bộ"');
						...
						$start = ($p-1)*$display;
						 $result = mysql_query("select * from san_pham where loai='Áo Câu Lạc Bộ' limit $start,$display") or die (mysql_error()); 
						$dem=0;
