...
<table id="table">
  <?php
			...
			if (isset($_GET['id']) && (int)$_GET['id']) $p=intval($_GET['id']);
					...
					$countqr=mysql_query('SELECT count(loai) FROM san_pham where loai="Áo Đội Tuyển"');
			...
			$start = ($p-1)*$display;
			$result = mysql_query("select * from san_pham where loai='Áo Đội Tuyển' limit $start,$display") or die (mysql_error()); 
			$dem=0;
